...
- When local analysis content changes, check whether the matching Confluence page, Jira story, or FigJam diagram also
  needs a sync.
- Before changing any task, feature, architecture, plan, or implementation, find the relevant canonical local source
  docs first, regardless of section. Check across `analysis/product`, `analysis/planning`, `analysis/technical`,
  `analysis/jira`, `analysis/confluence`, `analysis/design`, and `analysis/operations` as applicable, then use Jira,
  Confluence, and FigJam as synced execution, publication, or collaboration records.
