...
| ADRs | Strong local baseline | `technical/adr/0001` through `0011` | Add ADRs for media lifecycle, intake data, or future collaborator scope when decided. |
...
| CMS authoring UI is not implemented. | ADR 0011 defines the first owner-only source-aware boundary; implementation remains planned in `PF-206` and `PF-207`. |
...
| Media lifecycle decision | Decide upload, cleanup, alt text, and reconciliation rules before asset tooling. |
