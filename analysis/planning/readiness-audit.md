...
| Jira-compatible user stories | Jira story import started | `jira/user-stories.md`, `jira/epics.csv`, `jira/backlog.csv`, Jira epics `KAN-1` to `KAN-7`, Jira stories `KAN-8` to `KAN-19` | Continue importing remaining story batches and fill story mappings in `jira/import-history.md`. |
...
| Jira story import is not done yet. | Project `KAN` is accessible, epics `KAN-1` through `KAN-7` exist, and the `PF-E01`/`PF-E02` stories are mapped as `KAN-8` through `KAN-19`; thirty-six stories remain pending. |
