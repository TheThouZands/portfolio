...
| Implemented baseline | A usable current baseline exists and is externally recorded; future slices may extend it. |
...
| `FR-021` Service offers describe audience fit, scope boundaries, outputs, and proof surfaces. | Documented | `PERS-001`, `PERS-002`, `PERS-004` | `PF-701`, `PF-702`, `PF-703`, `PF-705` | `analysis/product/service-offer-catalog.md` | Offer catalog review, future service section and intake routing check. |
...
| `NFR-007` Portable docs for Confluence/Jira/FigJam. | Implemented baseline | owner, `PERS-002` | `PF-601`, `PF-602`, `PF-603`, `PF-608` | Local analysis suite, Confluence page manifest, Jira import history, FigJam section manifest | Markdown review, CSV parse, external publication records. |
...
| External tool portability | Current external baseline complete for Confluence, Jira, and FigJam. | Keep `PF-601`, `PF-602`, `PF-603`, and `PF-608` current when source records or publication state change. |
| Planned owner workflows | Story-backed but not implemented. | Moderation schema, owner authorization, CMS authoring, and media lifecycle directions are accepted in ADR 0009, ADR 0010, ADR 0011, and ADR 0012; build routes/actions only after the shared guard and focused tests exist. |
